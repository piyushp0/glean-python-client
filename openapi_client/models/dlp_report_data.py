...
class DlpReportData(BaseModel):
    ...
    """ # noqa: E501
    frequency: Optional[StrictStr] = Field(default=None, description="The frequency of the report")
    request_time: Optional[datetime] = Field(default=None, description="The time the report was requested, applicable only for one time reports", alias="requestTime")
    report_name: Optional[StrictStr] = Field(default=None, alias="reportName")
    status: Optional[DlpSimpleResult] = None
    __properties: ClassVar[List[str]] = ["frequency", "requestTime", "reportName", "status"]
    ...
    def frequency_validate_enum(cls, value):
        ...
        if value not in set(['WEEKLY', 'ONCE']):
            raise ValueError("must be one of enum values ('WEEKLY', 'ONCE')")
        return value
